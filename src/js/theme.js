		...
		function openLightbox( card ) {
			...
			if ( lbMedia ) {
				lbMedia.innerHTML = img
					...
					: '<div class="gallery-card__placeholder" style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:#E8E4DE"><span style="font-family:monospace;font-size:11px;letter-spacing:0.1em;text-transform:uppercase;color:rgba(26,26,26,0.3)">NO IMAGE</span></div>';
			}
			if ( lbTag )   { lbTag.textContent = tag ? tag.textContent : ''; }
			if ( lbTitle ) { lbTitle.textContent = title ? title.textContent : ''; }
			if ( lbVeh )   { lbVeh.textContent = vehicle ? vehicle.textContent : ''; }
